...
import (
	contracts_cookies "echo-starter/internal/contracts/cookies"
	"encoding/base64"
	"net/http"
	"reflect"
	"time"
...
func (s *service) Ctor() {
	hashKey, err := base64.StdEncoding.DecodeString(s.AppConfig.SecureCookieHashKey)
	if err != nil {
		panic(err)
	}
	encryptionKey, err := base64.StdEncoding.DecodeString(s.AppConfig.SecureCookieEncryptionKey)
	if err != nil {
		panic(err)
	}
